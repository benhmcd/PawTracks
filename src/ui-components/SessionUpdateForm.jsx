...
export default function SessionUpdateForm(props) {
  ...
  const resetStateValues = () => {
    ...
    setClip(
      typeof cleanValues.clip === "string"
        ? cleanValues.clip
        : JSON.stringify(cleanValues.clip)
    );
    setErrors({});
  };
  const [sessionRecord, setSessionRecord] = React.useState(session);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp ? await DataStore.query(Session, idProp) : session;
